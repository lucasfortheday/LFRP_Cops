...
if GetResourceMetadata(GetCurrentResourceName(), 'resource_Isdev', 0) == "yes" then
	RconPrint("/!\\ You are running a dev version of LFRP_Cops !\n")
end

if config.enableVersionNotifier then
	PerformHttpRequest("https://raw.githubusercontent.com/lucasfortheday/LFRP_Cops/master/police/__resource.lua", function(errorCode, result, headers)
		...
		if string.find(tostring(result), version) == nil then
			print("\n\r[LFRP_Cops] The version on this server is not up to date. Please update now.\n\r")
		end
	end, "GET", "", "")
end
